...
@Component({
  ...
  template: `<div class="container-fluid">
      <div class="row">
        <div class="col-sm-6">
        <h4>Kanban A</h4>
        <ejs-kanban id='KanbanA' #KanbanA keyField='Status' [dataSource]='dataA' [externalDropId]='externalKanbanADropId'  [showEmptyColumn]="true"
...
export class AppComponent {
  ...
  onKanbanADragStop(args: DragEventArgs) {
    let kanbanBElement: Element = <Element>(
      closest(args.event.target as Element, '#KanbanB')
    );
    if (kanbanBElement) {
  ...
  onKanbanBDragStop(args: DragEventArgs) {
    let kanbanAElement: Element = <Element>(
      closest(args.event.target as Element, '#KanbanA')
    );
    if (kanbanAElement) {
